...
case 335:
			...
			if (buttonId == 16){//accept
				p.tAccept[0] = true;
if(plr != null)
{
p.frames.setString(p, "Waiting for other player...", 335, 36);
p.frames.setString(plr, p.username+" has Accepted.", 335, 36);
}
				Engine.playerTrade.checkStage(p);
			}
			if (buttonId == 18){//decline
Engine.playerTrade.declineTrade(p);
if(plr != null)
{
Engine.playerTrade.declineTrade(plr);
}
			}
		break;
		case 334:

			if (buttonId == 20){//accept
				p.tAccept[1] = true;
				Engine.playerTrade.checkStage(p);
			}
			if (buttonId == 21){//decline
				Engine.playerTrade.declineTrade(p);
if(plr != null)
{
Engine.playerTrade.declineTrade(plr);
